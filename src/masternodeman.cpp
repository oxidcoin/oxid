...
CMasternode* CMasternodeMan::GetCurrentMasterNode(int mod, int64_t nBlockHeight, int minProtocol)
{
    ...
    BOOST_FOREACH(CMasternode& mn, vMasternodes) {
        mn.Check();
        LogPrintf("Is MN %s Enabled?: %i Status: %s Active State: %i\n", mn.addr.ToStringIPPort(), mn.IsEnabled(), mn.Status(), mn.activeState);
        if(mn.protocolVersion < minProtocol || !mn.IsEnabled()) continue;
    ...
    LogPrintf("Winner MN %s: Score: %i\n", winner->addr.ToStringIPPort(), winner->score);
    return winner;
}
	...
	if (strCommand == "dsee") { //DarkSend Election Entry
        ...
        if (pmn==NULL || pmn->tier >= 0) {
            BOOST_FOREACH(PAIRTYPE(const int, int) & mntier, masternodeCollateral)
            {
                ...
                //if (!fAcceptable && GetHeightAdjustedMasternodeReward(pindexBest->nHeight, mntier.second) == checkValue) {
                    CTransaction tx = CTransaction();

                    int64_t mn_collateral = GetMNCollateral(pindexBest->nHeight, mntier.first);
