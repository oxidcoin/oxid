...
bool CMasternodeDB::Write(const CMasternodeMan &mnodemanToSave) {
  ...
  ssMasternodes
      << strMagicMessage; // masternode cache file specific magic message
  ssMasternodes << FLATDATA(
      Params().MessageStart()); // network specific magic number
...
CMasternodeDB::ReadResult CMasternodeDB::Read(CMasternodeMan &mnodemanToLoad) {
  ...
  try {
    // de-serialize file header (masternode cache file specific magic message)
    // and ..
...
void DumpMasternodes() {
  ...
  // there was an error and it was not an error on file openning => do not
  // proceed
  if (readResult == CMasternodeDB::FileError)
    LogPrintf(
        "Missing masternode list file - mncache.dat, will try to recreate\n");
...
CMasternodeMan::CMasternodeMan() { nDsqCount = 0; }
...
void CMasternodeMan::AskForMN(CNode *pnode, CTxIn &vin) {
  std::map<COutPoint, int64_t>::iterator i =
      mWeAskedForMasternodeListEntry.find(vin.prevout);
...
void CMasternodeMan::CheckAndRemove() {
  ...
  map<COutPoint, int64_t>::iterator it2 =
      mWeAskedForMasternodeListEntry.begin();
...
void CMasternodeMan::DsegUpdate(CNode *pnode) {
  ...
  std::map<CNetAddr, int64_t>::iterator it =
      mWeAskedForMasternodeList.find(pnode->addr);
...
int64_t CMasternodeMan::GetAverageRewardTime(int minProtocol) {
  ...
  int count = 0;
  int64_t boundTime =
      2 * 24 * 60 * 60; // 2 days expressed in seconds, used to detect trash
                        // values if any in nLastPaid
  BOOST_FOREACH (CMasternode &mn, vMasternodes) {
    mn.Check();
...
CMasternode *
CMasternodeMan::FindRandomNotInVec(std::vector<CTxIn> &vecToExclude,
                                   int protocolVersion) {
  LOCK(cs);
  ...
  LogPrintf("CMasternodeMan::FindRandomNotInVec - nCountEnabled - "
            "vecToExclude.size() %d\n",
...
CMasternode *CMasternodeMan::GetCurrentMasterNode(int mod, int64_t nBlockHeight,
  ...
  BOOST_FOREACH (CMasternode &mn, vMasternodes) {
    ...
    LogPrintf("Masternode IP: %s Status: %s Active state: %i Tier: %i\n",
              mn.addr.ToStringIPPort(), mn.Status(), mn.activeState, mn.tier);
    ...
    if (n2 > score) {
      ...
      winner = &mn;
      LogPrintf(
          "Masternode IP: %s Status: %s Active state: %i Tier: %i Score: %i\n",
          mn.addr.ToStringIPPort(), mn.Status(), mn.activeState, mn.tier,
          score);
    }
  }
  ...
  if (winner) {
    ...
    ExtractDestination(GetScriptForDestination(winner->pubkey.GetID()),
                       address1);
    COxidcoinAddress address2(address1);
    LogPrintf(
        "Winner Masternode: %s Reward address: %s Reward percentage: %i\n",
        winner->addr.ToStringIPPort(), address2.ToString(),
        winner->rewardPercentage);
  }
...
int CMasternodeMan::GetMasternodeRank(const CTxIn &vin, int64_t nBlockHeight,
  ...
  sort(vecMasternodeScores.rbegin(), vecMasternodeScores.rend(),
       CompareValueOnly());
...
void CMasternodeMan::ProcessMessage(CNode *pfrom, std::string &strCommand,
                                    CDataStream &vRecv) {
  ...
  if (strCommand == "dsee") { // DarkSend Election Entry
    ...
    // 70047 and greater
    vRecv >> vin >> addr >> vchSig >> sigTime >> pubkey >> pubkey2 >> count >>
        current >> lastUpdated >> protocolVersion;

    // Invalid nodes check
    if (sigTime < 1532070000) {
    ...
    if (!darkSendSigner.VerifyMessage(pubkey, vchSig, strMessage,
                                      errorMessage)) {
    ...
    // masternodes with new dsee broadcasts
    CMasternode *pmn = this->Find(vin);
    // if we are a masternode but with undefined vin and this dsee is ours
    // (matches our Masternode privkey) then just skip this part
    if (pmn != NULL && !(fMasterNode && activeMasternode.vin == CTxIn() &&
                         pubkey2 == activeMasternode.pubKeyMasternode)) {
      // count == -1 when it's a new entry
      //   e.g. We don't want the entry relayed/time updated when we're syncing
      //   the list
      // mn.pubkey = pubkey, IsVinAssociatedWithPubkey is validated once below,
      //   after that they just need to match
      if (count == -1 && pmn->pubkey == pubkey &&
        ...
        if (pmn->sigTime < sigTime) { // take the newest entry
          if (!CheckNode((CAddress)addr)) {
            ...
            pmn->isPortOpen = true;
            addrman.Add(CAddress(addr), pfrom->addr,
                        2 * 60 * 60); // use this as a peer
          }
          ...
          pmn->isOldNode = true;
          if (pmn->IsEnabled())
            mnodeman.RelayOldMasternodeEntry(vin, addr, vchSig, sigTime, pubkey,
                                             pubkey2, count, current,
    ...
    // make sure the vout that was signed is related to the transaction that
    // spawned the masternode
    ...
    //  - this is checked later by .check() in many places and by
    ...
    if (pmn == NULL || pmn->tier >= 0) {
      BOOST_FOREACH (PAIRTYPE(const int, int) & mntier, masternodeCollateral) {
        if (!fAcceptable && (mntier.second * COIN) == checkValue) {
          // if (!fAcceptable &&
          // GetHeightAdjustedMasternodeReward(pindexBest->nHeight,
          // mntier.second) == checkValue) {
          CTransaction tx = CTransaction();

          int64_t mn_collateral =
              GetMNCollateral(pindexBest->nHeight, mntier.first);
          LogPrintf("Masternode Collateral: %i\n", mn_collateral);
          CTxOut vout =
              CTxOut(mn_collateral * COIN, darkSendPool.collateralPubKey);
      ...
      int64_t mn_collateral = GetMNCollateral(pindexBest->nHeight, pmn->tier);
      LogPrintf("mn_collateral: %i\n", mn_collateral);
      CTxOut vout = CTxOut(mn_collateral * COIN, darkSendPool.collateralPubKey);
      tx.vin.push_back(vin);
      tx.vout.push_back(vout);
      {
        TRY_LOCK(cs_main, lockMain);
    ...
    if (fAcceptable) {
      ...
      if (mi != mapBlockIndex.end() && (*mi).second) {
        ...
        CBlockIndex *pConfIndex = FindBlockByHeight(
            (pMNIndex->nHeight + MASTERNODE_MIN_CONFIRMATIONS -
             1)); // block where tx got MASTERNODE_MIN_CONFIRMATIONS
        if (pConfIndex->GetBlockTime() > sigTime) {
          LogPrintf("dsee - Bad sigTime %d for masternode %20s %105s (%i conf "
                    "block is at %d)\n",
      ...
      CMasternode mn(addr, vin, pubkey, vchSig, sigTime, pubkey2,
                     protocolVersion, rewardAddress, rewardPercentage);
      ...
      if (count == -1 && !isLocal)
        mnodeman.RelayOldMasternodeEntry(vin, addr, vchSig, sigTime, pubkey,
                                         pubkey2, count, current, lastUpdated,
                                         protocolVersion);
    ...
    vRecv >> vin >> addr >> vchSig >> sigTime >> pubkey >> pubkey2 >> count >>
        current >> lastUpdated >> protocolVersion >> rewardAddress >>
        rewardPercentage;
        ...
        if (pmn->sigTime < sigTime) { // take the newest entry
          ...
          if (pmn->IsEnabled())
            mnodeman.RelayMasternodeEntry(
                ...
                lastUpdated, protocolVersion, rewardAddress, rewardPercentage);
        }
      }
    ...
    if (pmn == NULL || pmn->tier >= 0) {
      BOOST_FOREACH (PAIRTYPE(const int, int) & mntier, masternodeCollateral) {
        if (!fAcceptable && (mntier.second * COIN) == checkValue) {
          CTransaction tx = CTransaction();
          CTxOut vout = CTxOut(
              (GetMNCollateral(pindexBest->nHeight, mntier.first)) * COIN,
      ...
      CTxOut vout =
          CTxOut((GetMNCollateral(pindexBest->nHeight, pmn->tier)) * COIN,
                 darkSendPool.collateralPubKey);
      tx.vin.push_back(vin);
      tx.vout.push_back(vout);
      {
    ...
    if (pmn != NULL && pmn->protocolVersion >= MIN_POOL_PEER_PROTO_VERSION) {
      ...
      if (pmn->lastDseep < sigTime) {
        std::string strMessage = pmn->addr.ToString() +
                                 boost::lexical_cast<std::string>(sigTime) +
                                 boost::lexical_cast<std::string>(stop);

        std::string errorMessage = "";
        if (!darkSendSigner.VerifyMessage(pmn->pubkey2, vchSig, strMessage,
                                          errorMessage)) {
    ...
    if (pmn != NULL) {
      if ((GetAdjustedTime() - pmn->lastVote) > (60 * 60)) {
        std::string strMessage =
            vin.ToString() + boost::lexical_cast<std::string>(nVote);
    ...
    if (vin == CTxIn()) { // only should ask for this once
      ...
      if (!pfrom->addr.IsRFC1918() &&
        ...
        std::map<CNetAddr, int64_t>::iterator i =
            mAskedUsForMasternodeList.find(pfrom->addr);
    ...
    BOOST_FOREACH (CMasternode &mn, vMasternodes) {
      ...
      if (mn.IsEnabled()) {
        ...
        if (vin == CTxIn()) {
          if (mn.isOldNode) {
            pfrom->PushMessage("dsee", mn.vin, mn.addr, mn.sig, mn.sigTime,
                               mn.pubkey, mn.pubkey2, count, i, mn.lastTimeSeen,
                               mn.protocolVersion);
          } else {
            pfrom->PushMessage("dsee+", mn.vin, mn.addr, mn.sig, mn.sigTime,
                               mn.pubkey, mn.pubkey2, count, i, mn.lastTimeSeen,
                               mn.protocolVersion, mn.rewardAddress,
                               mn.rewardPercentage);
          }
        } else if (vin == mn.vin) {
          if (mn.isOldNode) {
...
void CMasternodeMan::RelayOldMasternodeEntry(
    const CTxIn vin, const CService addr,
  ...
  LOCK(cs_vNodes);
  BOOST_FOREACH (CNode *pnode, vNodes) {
    pnode->PushMessage("dsee", vin, addr, vchSig, nNow, pubkey, pubkey2, count,
                       current, lastUpdated, protocolVersion);
  }
}

void CMasternodeMan::RelayMasternodeEntry(
    const CTxIn vin, const CService addr,
    const std::vector<unsigned char> vchSig, const int64_t nNow,
    const CPubKey pubkey, const CPubKey pubkey2, const int count,
    const int current, const int64_t lastUpdated, const int protocolVersion,
...
void CMasternodeMan::RelayMasternodeEntryPing(
    const CTxIn vin, const std::vector<unsigned char> vchSig,
