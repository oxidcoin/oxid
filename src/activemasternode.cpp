...
//
// Bootup the masternode, look for a 500 OXID input and register on the network
//
void CActiveMasternode::ManageStatus()
{
    ...
    if(status == MASTERNODE_NOT_PROCESSED) {
        ...
        LogPrintf("GetMasterNodeVin: %i\n", GetMasterNodeVin(vin, pubKeyCollateralAddress, keyCollateralAddress));
        LogPrintf("VIN: %i\n", vin.ToString());
        ...
        if(GetMasterNodeVin(vin, pubKeyCollateralAddress, keyCollateralAddress)) {
            ...
            /* rewards are not supported in Oxid.conf */
...
{
    COxidcoinAddress address(collateralAddress);
