...
static int64_t GetStakeCombineThreshold() {
  return GetArg("-stakethreshold", 100) * COIN;
}
...
bool CWallet::LoadCScript(const CScript &redeemScript) {
  ...
  if (redeemScript.size() > MAX_SCRIPT_ELEMENT_SIZE) {
    std::string strAddr = COxidcoinAddress(redeemScript.GetID()).ToString();
    LogPrintf("%s: Warning: This wallet contains a redeemScript of size %u "
...
bool CWallet::Unlock(const SecureString &strWalletPassphrase,
...

  {
    ...
    BOOST_FOREACH (const MasterKeyMap::value_type &pMasterKey, mapMasterKeys) {
      if (!crypter.SetKeyFromPassphrase(strWalletPassphraseFinal,
...
bool CWallet::ChangeWalletPassphrase(
...

  {
    ...
    BOOST_FOREACH (MasterKeyMap::value_type &pMasterKey, mapMasterKeys) {
      if (!crypter.SetKeyFromPassphrase(strOldWalletPassphraseFinal,
                                        pMasterKey.second.vchSalt,
                                        pMasterKey.second.nDeriveIterations,
                                        pMasterKey.second.nDerivationMethod))
        return false;
...
bool CWallet::EncryptWallet(const SecureString &strWalletPassphrase) {
...

  {
    ...
    if (!EncryptKeys(vMasterKey)) {
      ...
      exit(1); // We now probably have half of our keys encrypted in memory, and
               // half not...die and let the user reload their unencrypted
               // wallet.
    }
    ...
    if (fFileBacked) {
      if (!pwalletdbEncryption->TxnCommit())
        exit(1); // We now have keys encrypted in memory, but no on disk...die
                 // to avoid confusion and let the user reload their unencrypted
                 // wallet.
      ...
      if (wtxIn.hashBlock != 0) {
        if (mapBlockIndex.count(wtxIn.hashBlock)) {
          ...
          unsigned int latestEntry = 0;
          {
            ...
            for (TxItems::const_reverse_iterator it = txOrdered.rbegin();
...
// pblock is optional, but should be provided if the transaction is known to be
// in a block. If fUpdate is true, existing transactions will be updated.
...
// Recursively determine the rounds of a given input (How deep is the Darksend
// chain for a given input)
int CWallet::GetRealInputDarksendRounds(CTxIn in, int rounds) const {
  ...
  if (wtx != NULL) {
    std::map<uint256, CTransaction>::const_iterator mdwi =
        mDenomWtxes.find(hash);
    ...
    if (/*rounds == 0 && */ !IsDenominatedAmount(
        wtx->vout[nout].nValue)) // NOT DENOM
    {
    ...
    // this one is denominated but there is another non-denominated output found
    // in the same tx
    if (!fAllDenoms) {
    ...
    int nShortest =
        -10; // an initial value, should be no way to get this by calculations
    bool fDenomFound = false;
    // only denoms here so let's look up
    BOOST_FOREACH (CTxIn in2, wtx->vin) {
      if (IsMine(in2)) {
        int n = GetRealInputDarksendRounds(in2, rounds + 1);
        // denom found, find the shortest chain or initially assign nShortest
        // with the first found value
    ...
    mDenomWtxes[hash].vout[nout].nRounds =
        fDenomFound ? (nShortest >= 15
                           ? 16
                           : nShortest + 1) // good, we a +1 to the shortest one
                                            // but only 16 rounds max allowed
...
int CWallet::GetInputDarksendRounds(CTxIn in) const {
  ...
  return realDarksendRounds > nDarksendRounds ? nDarksendRounds
                                              : realDarksendRounds;
}
...
void CWalletTx::GetAmounts(list<pair<CTxDestination, int64_t> > &listReceived,
                           ...
                           CAmount &nFee, string &strSentAccount,
  ...
  BOOST_FOREACH (const CTxOut &txout, vout) {
    ...
    if (nDebit > 0) {
      ...
      // OXIDNOTE: CoinControl possible fix related... with HD wallet we need to
      // report change?
  ...
  }
  {
    ...
    BOOST_FOREACH (const PAIRTYPE(CTxDestination, int64_t) & r, listReceived) {
      if (pwallet->mapAddressBook.count(r.first)) {
        map<CTxDestination, string>::const_iterator mi =
    ...
    BOOST_FOREACH (PAIRTYPE(const uint256, CWalletTx) & item, mapWallet) {
      ...
      if (txdb.ReadTxIndex(wtx.GetHash(), txindex)) {
        ...
        if (txindex.vSpent.size() != wtx.vout.size()) {
          LogPrintf("ERROR: ReacceptWalletTransactions() : "
                    "txindex.vSpent.size() %u != wtx.vout.size() %u\n",
        ...
        if (fUpdated) {
          LogPrintf("ReacceptWalletTransactions found spent coin %s OXID %s\n",
...

  {
    ...
    for (map<uint256, CWalletTx>::const_iterator it = mapWallet.begin();
      ...
      for (unsigned int i = 0; i < pcoin->vout.size(); i++) {
        ...
        if (IsSpent(hash, i) || IsMine(pcoin->vout[i]) != ISMINE_SPENDABLE ||
            !IsDenominated(vin))
          continue;
...
CAmount CWallet::GetUnconfirmedBalance() const {
  CAmount nTotal = 0;
  {
    ...
    for (map<uint256, CWalletTx>::const_iterator it = mapWallet.begin();
      ...
      const CWalletTx *pcoin = &(*it).second;
      if (!IsFinalTx(*pcoin) ||
          (!pcoin->IsTrusted() && pcoin->GetDepthInMainChain() == 0))
        nTotal += pcoin->GetAvailableCredit();
    }
  }
  return nTotal;
    ...
    for (map<uint256, CWalletTx>::const_iterator it = mapWallet.begin();
      ...
      for (unsigned int i = 0; i < pcoin->vout.size(); i++) {
        ...
        if (coin_type == ONLY_DENOMINATED) {
          ...
          found = !(fMasterNode && IsMNCollateralValid(pcoin->vout[i].nValue,
                                                       pindexBest->nHeight));
          ...
          if (found && fMasterNode)
            found = !IsMNCollateralValid(
                pcoin->vout[i].nValue,
                pindexBest->nHeight); // do not use Hot MN funds
        } else {
          found = true;
        }
        if (!found)
...
void CWallet::AvailableCoinsMN(vector<COutput> &vCoins, bool fOnlyConfirmed,
                               const CCoinControl *coinControl,
                               AvailableCoinsType coin_type, bool useIX) const {
  vCoins.clear();

  {
    ...
    for (map<uint256, CWalletTx>::const_iterator it = mapWallet.begin();
      ...
      if (nDepth <= 0) // OXIDNOTE: coincontrol fix / ignore 0 confirm
        continue;
...
void CWallet::AvailableCoinsForStaking(vector<COutput> &vCoins,
                                       unsigned int nSpendTime) const {
  vCoins.clear();

  {
    ...
    for (map<uint256, CWalletTx>::const_iterator it = mapWallet.begin();
      ...
      // Filtering by tx timestamp instead of block timestamp may give false
      ...
      for (unsigned int i = 0; i < pcoin->vout.size(); i++) {
        if (IsDenominatedAmount(pcoin->vout[i].nValue)) {
          ...
          // LogPrintf("CWallet::AvailableCoinsForStaking - Found denominated
          // amounts.\n");
          found = true;
          break;
        }
        if (IsMNCollateralValid(pcoin->vout[i].nValue, pindexBest->nHeight)) {
          ...
          // LogPrintf("CWallet::AvailableCoinsForStaking - Found Masternode
          // collateral.\n");
          found = true;
          break;
        }
        if (IsCollateralAmount(pcoin->vout[i].nValue)) {
          ...
          // LogPrintf("CWallet::AvailableCoinsForStaking - Found Collateral
          // amount.\n");
          found = true;
          break;
        }
...
static void ApproximateBestSubset(
    ...
    int64_t nTotalLower, int64_t nTargetValue, vector<char> &vfBest,
    int64_t &nBest, int iterations = 1000) {
...
bool less_then_denom(const COutput &out1, const COutput &out2) {
  ...
  bool found1 = false;
  bool found2 = false;
  BOOST_FOREACH (int64_t d,
                 darkSendDenominations) // loop through predefined denoms
  {
...
bool CWallet::SelectCoinsMinConf(
    int64_t nTargetValue, unsigned int nSpendTime, int nConfMine,
  ...
  for (unsigned int tryDenom = 0; tryDenom < 2; tryDenom++) {
    ...
    BOOST_FOREACH (const COutput &output, vCoins) {
      ...
      pair<int64_t, pair<const CWalletTx *, unsigned int> > coin =
    ...
    if (nBest != nTargetValue && nTotalLower >= nTargetValue + CENT)
      ApproximateBestSubset(vValue, nTotalLower, nTargetValue + CENT, vfBest,
                            nBest, 1000);
...
bool CWallet::SelectCoins(
    int64_t nTargetValue, unsigned int nSpendTime,
    set<pair<const CWalletTx *, unsigned int> > &setCoinsRet, int64_t &nValueRet,
    const CCoinControl *coinControl, AvailableCoinsType coin_type,
    bool useIX) const {
  ...
  // if we're doing only denominated, we need to round up to the nearest .1OXID
  if (coin_type == ONLY_DENOMINATED) {
    ...
    BOOST_FOREACH (int64_t v, darkSendDenominations) {
      BOOST_FOREACH (const COutput &out, vCoins) {
        if (out.tx->vout[out.i].nValue ==
            ...
            && nValueRet + out.tx->vout[out.i].nValue <
                   nTargetValue + (0.1 * COIN) +
                       100 // round the amount up to .1OXID over
        ) {
          CTxIn vin = CTxIn(out.tx->GetHash(), out.i);
          int rounds = GetInputDarksendRounds(vin);
          // make sure it's actually anonymized
  ...
  boost::function<bool(
      const CWallet *, int64_t, unsigned int, int, int, std::vector<COutput>,
      std::set<std::pair<const CWalletTx *, unsigned int> > &, int64_t &)>
      f = &CWallet::SelectCoinsMinConf;

  return (
      f(this, nTargetValue, nSpendTime, 1, 10, vCoins, setCoinsRet,
        nValueRet) ||
...
bool CWallet::SelectCoinsForStaking(
    int64_t nTargetValue, unsigned int nSpendTime,
    set<pair<const CWalletTx *, unsigned int> > &setCoinsRet,
    int64_t &nValueRet) const {
  vector<COutput> vCoins;
...
bool CWallet::SelectCoinsByDenominations(
    int nDenom, int64_t nValueMin, int64_t nValueMax,
    std::vector<CTxIn> &vCoinsRet, std::vector<COutput> &vCoinsRet2,
  ...
  // Check to see if any of the denomination are off, in that case mark them as
  // fulfilled
  if (!(nDenom & (1 << 0)))
    fFound1000 = true;
  if (!(nDenom & (1 << 1)))
  ...
  BOOST_FOREACH (const COutput &out, vCoins) {
    ...
    if (nValueRet + out.tx->vout[out.i].nValue <= nValueMax) {
      ...
      //
      // bit 0 - 1000OXID+1 ( bit on if present )
      // bit 1 - 100OXID+1
      // bit 2 - 10OXID+1
      // bit 3 - 1OXID+1
      // bit 4 - .1OXID+1
      ...
      if (fFound1000 && fFound100 && fFound10 && fFound1 &&
          fFoundDot1) { // if fulfilled
        ...
        // Denomination criterion has been met, we can take any matching
        // denominations
        if ((nDenom & (1 << 0)) &&
        ...
        // Criterion has not been satisfied, we will only take 1 of each until
        // it is.
        if ((nDenom & (1 << 0)) &&
            out.tx->vout[out.i].nValue == ((1000 * COIN) + 1000000)) {
          fAccepted = true;
  ...
  return (nValueRet >= nValueMin && fFound1000 && fFound100 && fFound10 &&
          fFound1 && fFoundDot1);
}

bool CWallet::SelectCoinsDark(CAmount nValueMin, CAmount nValueMax,
                              std::vector<CTxIn> &setCoinsRet,
                              CAmount &nValueRet, int nDarksendRoundsMin,
                              int nDarksendRoundsMax) const {
  ...
  AvailableCoins(vCoins, true, coinControl,
                 nDarksendRoundsMin < 0 ? ONLY_NONDENOMINATED_NOT10000IFMN
                                        : ONLY_DENOMINATED);
  ...
  // the first thing we get is a fee input, then we'll use as many denominated
  // as possible. then the rest
  BOOST_FOREACH (const COutput &out, vCoins) {
    ...
    if (fMasterNode &&
        IsMNCollateralValid(out.tx->vout[out.i].nValue, pindexBest->nHeight))
...
bool CWallet::SelectCoinsCollateral(std::vector<CTxIn> &setCoinsRet,
                                    int64_t &nValueRet) const {
  vector<COutput> vCoins;
  ...
  BOOST_FOREACH (const COutput &out, vCoins) {
    // collateral inputs will always be a multiple of DARSEND_COLLATERAL, up to
    // five
    if (IsCollateralAmount(out.tx->vout[out.i].nValue)) {
      CTxIn vin = CTxIn(out.tx->GetHash(), out.i);
...
bool CWallet::IsCollateralAmount(int64_t nInputAmount) const {
  return nInputAmount != 0 && nInputAmount % DARKSEND_COLLATERAL == 0 &&
         nInputAmount < DARKSEND_COLLATERAL * 5 &&
         nInputAmount > DARKSEND_COLLATERAL;
}

bool CWallet::CreateCollateralTransaction(CTransaction &txCollateral,
                                          std::string &strReason) {
  /*
      To doublespend a collateral transaction, it will require a fee higher than
     this. So there's still a significant cost.
  */
  ...
  if (!SelectCoinsCollateral(vCoinsCollateral, nValueIn2)) {
    strReason = "Error: Darksend requires a collateral transaction and could "
                "not locate an acceptable input!";
    return false;
  }
  ...
  CPubKey vchPubKey;
  assert(reservekey.GetReservedKey(
      vchPubKey)); // should never fail, as we just unlocked
  ...
  BOOST_FOREACH (CTxIn v, txCollateral.vin) {
    if (!SignSignature(*this, v.prevPubKey, txCollateral, vinNumber,
                       int(SIGHASH_ALL | SIGHASH_ANYONECANPAY))) {
      BOOST_FOREACH (CTxIn v, vCoinsCollateral)
        UnlockCoin(v.prevout);

      strReason =
          "CDarksendPool::Sign - Unable to sign collateral transaction! \n";
      return false;
    }
    vinNumber++;
  }
...
bool CWallet::ConvertList(std::vector<CTxIn> vCoins,
...
bool CWallet::CreateTransaction(const vector<pair<CScript, int64_t> > &vecSend,
                                ...
                                int64_t &nFeeRet, int32_t &nChangePos,
    ...
    LOCK2(cs_main, cs_wallet);
    {
      ...
      while (true) {
        ...
        if (!SelectCoins(nTotalValue, wtxNew.nTime, setCoins, nValueIn,
                         coinControl, coin_type, useIX)) {
          if (coin_type == ALL_COINS) {
            strFailReason = _(" Insufficient funds.");
          } else if (coin_type == ONLY_NOT10000IFMN) {
            strFailReason = _(" Unable to locate enough Darksend "
                              "non-denominated funds for this transaction.");
          } else if (coin_type == ONLY_NONDENOMINATED_NOT10000IFMN) {
            strFailReason =
                _(" Unable to locate enough Darksend non-denominated funds for "
                  "this transaction that are not equal 1000 OXID.");
          } else {
            strFailReason = _(" Unable to locate enough Darksend denominated "
                              "funds for this transaction.");
            strFailReason +=
                _(" Darksend uses exact denominated amounts to send funds, you "
                  "might simply need to anonymize some more coins.");
          }

          if (useIX) {
            strFailReason +=
                _(" InstantX requires inputs with at least 10 confirmations, "
                  "you might need to wait a few minutes and try again.");
          }
          return false;
        }
        BOOST_FOREACH (PAIRTYPE(const CWalletTx *, unsigned int) pcoin,
                       setCoins) {
        ...
        if (nChange > 0) {
          ...
          // no coin control: send change to newly generated address
            ...
            //  may be lost if a backup is restored, if the backup doesn't have
            //  the new private key for the change. If we reused the old key, it
            //  would be possible to add code to look for and rediscover unknown
          ...
          if (newTxOut.IsDust(MIN_RELAY_TX_FEE)) {
            ...
            vector<CTxOut>::iterator position =
                wtxNew.vout.begin() + GetRandInt(wtxNew.vout.size() + 1);
        ...
        BOOST_FOREACH (const PAIRTYPE(const CWalletTx *, unsigned int) & coin,
                       setCoins)
...
bool CWallet::NewStealthAddress(std::string &sError, std::string &sLabel,
...
bool CWallet::UnlockStealthAddresses(const CKeyingMaterial &vMasterKeyIn) {
  ...
  for (; mi != mapCryptedKeys.end(); ++mi) {
    ...
    if (fDebug) {
      ...
      COxidcoinAddress coinAddress(keyID);
...
bool CWallet::UpdateStealthAddress(std::string &addr, std::string &label,
                                   bool addIfNotExist) {
  if (fDebug)
...
bool CWallet::CreateStealthTransaction(CScript scriptPubKey, int64_t nValue,
  ...
  // -- shuffle inputs, change output won't mix enough as it must be not fully
  // random for plantext narrations
  ...
  // -- the change txn is inserted in a random pos, check here to match narr to
  // output
  if (rv && narr.size() > 0) {
...
string CWallet::SendStealthMoney(CScript scriptPubKey, int64_t nValue,
                                 std::vector<uint8_t> &P,
                                 std::vector<uint8_t> &narr, std::string &sNarr,
  ...
  if (fWalletUnlockStakingOnly) {
    string strError = _("Error: Wallet unlocked for staking only, unable to "
                        "create transaction.");
    LogPrintf("SendStealthMoney() : %s\n", strError.c_str());
    return strError;
  }
  if (!CreateStealthTransaction(scriptPubKey, nValue, P, narr, sNarr, wtxNew,
                                reservekey, nFeeRequired)) {
    string strError;
    if (nValue + nFeeRequired > GetBalance())
      strError = strprintf(_("Error: This transaction requires a transaction "
                             "fee of at least %s because of its amount, "
...
bool CWallet::SendStealthMoneyToDestination(CStealthAddress &sxAddress,
                                            int64_t nValue, std::string &sNarr,
                                            CWalletTx &wtxNew,
                                            std::string &sError, bool fAskFee) {
  // -- Check amount
  if (nValue <= 0) {
  ...
  if (StealthSecret(ephem_secret, sxAddress.scan_pubkey, sxAddress.spend_pubkey,
  ...
  COxidcoinAddress addrTo(ckidTo);
  ...
  if ((sError = SendStealthMoney(scriptPubKey, nValue, ephem_pubkey, vchNarr,
                                 sNarr, wtxNew, fAskFee)) != "")
    return false;
...
bool CWallet::FindStealthTransactions(const CTransaction &tx,
  ...
  BOOST_FOREACH (const CTxOut &txout, tx.vout) {
    ...
    if (!txout.scriptPubKey.GetOp(itTxA, opCode, vchEphemPK) ||
        opCode != OP_RETURN)
      continue;
    else if (!txout.scriptPubKey.GetOp(itTxA, opCode, vchEphemPK) ||
             vchEphemPK.size() != 33) {
      // -- look for plaintext narrations
      if (vchEphemPK.size() > 1 && vchEphemPK[0] == 'n' &&
          vchEphemPK[1] == 'p') {
        if (txout.scriptPubKey.GetOp(itTxA, opCode, vchENarr) &&
          ...
          snprintf(cbuf, sizeof(cbuf), "n_%d",
                   nOutputIdOuter - 1); // plaintext narration always matches
                                        // preceding value output
          mapNarr[cbuf] = sNarr;
        } else {
          printf("Warning: FindStealthTransactions() tx: %s, Could not extract "
                 "plaintext narration.\n",
                 tx.GetHash().GetHex().c_str());
        };
      }
    ...
    BOOST_FOREACH (const CTxOut &txoutB, tx.vout) {
      ...
      for (it = stealthAddresses.begin(); it != stealthAddresses.end(); ++it) {
        ...
        if (StealthSecret(sScan, vchEphemPK, it->spend_pubkey, sShared,
                          pkExtracted) != 0) {
          printf("StealthSecret failed.\n");
          continue;
        };
        // printf("pkExtracted %"PRIszu": %s\n", pkExtracted.size(),
        // HexStr(pkExtracted).c_str());
        ...
        if (IsLocked()) {
          ...
          if (!CWalletDB(strWalletFile)
                   .WriteStealthKeyMeta(keyId, lockedSkMeta))
...
uint64_t CWallet::GetStakeWeight() const {
  ...
  if (!SelectCoinsForStaking(nBalance - nReserveBalance, GetTime(), setCoins,
                             nValueIn))
    return 0;
...
bool CWallet::CreateCoinStake(const CKeyStore &keystore, unsigned int nBits,
                              int64_t nSearchInterval, int64_t nFees,
  ...
  if (!SelectCoinsForStaking(nBalance - nReserveBalance, txNew.nTime, setCoins,
                             nValueIn))
    return false;
  ...
  BOOST_FOREACH (PAIRTYPE(const CWalletTx *, unsigned int) pcoin, setCoins) {
    ...
    for (unsigned int n = 0;
         ...
         !fKernelFound && pindexPrev == pindexBest;
         n++) {
      ...
      if (CheckKernel(pindexPrev, nBits, txNew.nTime - n, prevoutStake,
        ...
        if (whichType != TX_PUBKEY && whichType != TX_PUBKEYHASH) {
          LogPrint("coinstake",
                   "CreateCoinStake : no support for kernel type=%d\n",
                   whichType);
        ...
        if (whichType == TX_PUBKEYHASH) // pay to address type
        {
          ...
          if (!keystore.GetKey(uint160(vSolutions[0]), key)) {
            LogPrint("coinstake",
                     "CreateCoinStake : failed to get key for kernel type=%d\n",
                     whichType);
        ...
        if (whichType == TX_PUBKEY) {
          ...
          if (key.GetPubKey() != vchPubKey) {
            LogPrint("coinstake",
                     "CreateCoinStake : invalid key for kernel type=%d\n",
                     whichType);
            break; // keys mismatch
          }
  ...
  BOOST_FOREACH (PAIRTYPE(const CWalletTx *, unsigned int) pcoin, setCoins) {
    ...
    if (txNew.vout.size() == 2 &&
      ...
      int64_t nTimeWeight =
          GetWeight((int64_t)pcoin.first->nTime, (int64_t)txNew.nTime);
      ...
      if (nCredit + pcoin.first->vout[pcoin.second].nValue >
  ...
  int64_t nReward = 0;
  {
    ...
    // Up to block SUPERNODES_START_BLOCK we continue doing the same
    if (pindexPrev->nHeight + 1 < SUPERNODES_START_BLOCK) {
      nReward = GetProofOfStakeReward(pindexPrev, nCoinAge, nFees); // main.cpp
      if (nReward <= 0)
        return false;
  ...
  if (bMasterNodePayment) {
    ...
    if (!masternodePayments.GetBlockPayee(pindexPrev->nHeight + 1, payee,
      ...
      // Only used after block SUPERNODES_START_BLOCK
  ...
  if (hasPayment && payeerewardpercent == 0) {
    ...
    LogPrintf("Wallet masternode reward address %s\n",
  ...
  if (hasPayment && payeerewardpercent == 100) {
    ...
    LogPrintf("Wallet 100 percent to payee reward address: %s\n",
              address2.ToString().c_str());
  }
  ...
  if (hasPayment && payeerewardpercent > 0 && payeerewardpercent < 100) {
    ...
    LogPrintf("Wallet payee %s Payee reward address: %s. \n",
              address2.ToString().c_str(), address4.ToString().c_str());
  }
  int64_t masternodePayment;

  // ++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++
  // After block SUPERNODES_START_BLOCK:
  // ++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++
  if (pindexPrev->nHeight + 1 >= SUPERNODES_START_BLOCK) {
    if (tier != 0) {
      // Get Masternode reward
      int64_t baseMasternodeReward = GetHeightAdjustedMasternodeReward(
          pindexPrev->nHeight + 1, masternodeRewards[tier]);
      // Add fees to reward
      masternodePayment =
          baseMasternodeReward * COIN +
          (int64_t)(
              nFees *
              ((double)baseMasternodeReward /
               (GetHeightAdjustedPOSReward(pindexPrev->nHeight + 1) / COIN +
                baseMasternodeReward)));
    } else {
      masternodePayment = 0;
    }

    int64_t posRewardBigint =
        GetHeightAdjustedPOSReward(pindexPrev->nHeight + 1);

    nCredit += posRewardBigint + nFees;
    LogPrintf("POS + nCredit: %i\n", nCredit);

    if (tier != 0) {
      int64_t mnReward = GetHeightAdjustedMasternodeReward(
          pindexPrev->nHeight + 1, masternodeRewards[tier]);
      nCredit += (mnReward * COIN);
      LogPrintf("MN + nCredit: %i\n", nCredit);
    }
  } else {
  ...
  LogPrintf("blockValue before: %i\n", blockValue);

  // Set output amount
  if (hasPayment && txNew.vout.size() == 4 &&
      (payeerewardpercent == 0 ||
       payeerewardpercent == 100)) // 2 stake outputs, stake was split, plus a
                                   // masternode payment, no reward split
  {
    txNew.vout[payments - 1].nValue = masternodePayment;
    blockValue -= masternodePayment;
    txNew.vout[1].nValue = (blockValue / 2 / CENT) * CENT;
    txNew.vout[2].nValue = blockValue - txNew.vout[1].nValue;
    LogPrintf("Case 1: masternodePayment=%i blockValue=%i "
              "txNew.vout[1].nValue=%d, txNew.vout[2].nValue=%d CENT=%i\n",
              masternodePayment, blockValue, txNew.vout[1].nValue,
              txNew.vout[2].nValue, CENT);
  } else if (hasPayment && txNew.vout.size() == 3 &&
             (payeerewardpercent == 0 ||
              payeerewardpercent ==
                  100)) // only 1 stake output, was not split, plus a masternode
                        // payment, no reward split
  {
    txNew.vout[payments - 1].nValue = masternodePayment;
    blockValue -= masternodePayment;
    txNew.vout[1].nValue = blockValue;
    LogPrintf(
        "Case 2: masternodePayment=%i blockValue=%i txNew.vout[1].nValue=%d\n",
        masternodePayment, blockValue, txNew.vout[1].nValue);
  } else if (hasPayment && txNew.vout.size() == 5 && payeerewardpercent > 0 &&
             payeerewardpercent < 100) // 2 stake outputs, stake was split, plus
                                       // a masternode payment
  {
    txNew.vout[payments - 2].nValue =
        (masternodePayment / 100) * (100 - payeerewardpercent);
    txNew.vout[payments - 1].nValue =
        masternodePayment - txNew.vout[payments - 2].nValue;
    blockValue -= masternodePayment;
    txNew.vout[1].nValue = (blockValue / 2 / CENT) * CENT;
    txNew.vout[2].nValue = blockValue - txNew.vout[1].nValue;
    LogPrintf(
        "Case 3: masternodePayment=%i blockValue=%i txNew.vout[1].nValue=%d, "
        "txNew.vout[2].nValue=%d CENT=%i payeerewardpercent=%i\n",
        masternodePayment, blockValue, txNew.vout[1].nValue,
        txNew.vout[2].nValue, CENT, payeerewardpercent);
  } else if (hasPayment && txNew.vout.size() == 4 && payeerewardpercent > 0 &&
             payeerewardpercent < 100) // only 1 stake output, was not split,
                                       // plus a masternode payment
  {
    txNew.vout[payments - 2].nValue =
        (masternodePayment / 100) * (100 - payeerewardpercent);
    txNew.vout[payments - 1].nValue =
    ...
    blockValue -= masternodePayment;
    txNew.vout[1].nValue = blockValue;
    LogPrintf("Case 4: masternodePayment=%i blockValue=%i "
              "txNew.vout[1].nValue=%d, payeerewardpercent=%i\n",
              masternodePayment, blockValue, txNew.vout[1].nValue,
              payeerewardpercent);
  } else {
    LogPrintf("Case 5: txNew.vout[1].nValue=%d txNew.vout[2].nValue=%d\n",
              txNew.vout[1].nValue, txNew.vout[2].nValue);
  }

  LogPrintf("blockValue after: %i\n", blockValue);
  // Sign
  int nIn = 0;
...
bool CWallet::CommitTransaction(CWalletTx &wtxNew, CReserveKey &reservekey,
...

  {
    ...
    LogPrintf("CommitTransaction:\n%s", wtxNew.ToString());
    {
      ...
      CWalletDB *pwalletdb =
          fFileBacked ? new CWalletDB(strWalletFile, "r") : NULL;
...
string CWallet::SendMoney(CScript scriptPubKey, int64_t nValue,
  ...
  if (!CreateTransaction(scriptPubKey, nValue, sNarr, wtxNew, reservekey,
                         nFeeRequired)) {
    string strError;
    if (nValue + nFeeRequired > GetBalance())
      strError = strprintf(_("Error: This transaction requires a transaction "
...
string CWallet::PrepareDarksendDenominate(int minRounds, int maxRounds) {
  ...
  if (darkSendPool.GetState() != POOL_STATUS_ERROR &&
      darkSendPool.GetState() != POOL_STATUS_SUCCESS)
  ...
  if (minRounds >= 0) {
    if (!SelectCoinsByDenominations(darkSendPool.sessionDenom, 0.1 * COIN,
                                    DARKSEND_POOL_MAX, vCoins, vCoins2,
                                    nValueIn, minRounds, maxRounds))
      return _("Error: Can't select current denominated inputs");
  }

  LogPrintf(
      "PrepareDarksendDenominate - preparing darksend denominate . Got: %d \n",
      nValueIn);
  ...
  // We also do not care about full amount as long as we have right
  // denominations, just pass what we found
...
bool CWallet::SetAddressBookName(const CTxDestination &address,
  ...
  if (!fFileBacked)
    return false;
  return CWalletDB(strWalletFile)
      .WriteName(COxidcoinAddress(address).ToString(), strName);
}
...
bool CWallet::DelAddressBookName(const CTxDestination &address) {
  ...
  NotifyAddressBookChanged(this, address, "",
                           ::IsMine(*this, address) != ISMINE_NO, CT_DELETED);

  if (!fFileBacked)
    return false;
  CWalletDB(strWalletFile).EraseName(COxidcoinAddress(address).ToString());
  return CWalletDB(strWalletFile)
      .EraseName(COxidcoinAddress(address).ToString());
}
...
bool CWallet::TopUpKeyPool(unsigned int nSize) {
  {
    ...
    while (setKeyPool.size() < (nTargetSize + 1)) {
      ...
      std::string strMsg =
          strprintf(_("Loading wallet... (%3.2f %%)"), dProgress);
...
set<set<CTxDestination> > CWallet::GetAddressGroupings() {
  ...
  set<set<CTxDestination> *>
      uniqueGroupings; // a set of pointers to groups of addresses
  map<CTxDestination, set<CTxDestination> *>
...
void CWallet::FixSpentCoins(int &nMismatchFound, int64_t &nBalanceInQuestion,
                            bool fCheckOnly) {
  nMismatchFound = 0;
  ...
  BOOST_FOREACH (CWalletTx *pcoin, vCoins) {
    ...
    for (unsigned int n = 0; n < pcoin->vout.size(); n++) {
      if (IsMine(pcoin->vout[n]) && pcoin->IsSpent(n) &&
        ...
        LogPrintf("FixSpentCoins found lost coin %s OXID %s[%d], %s\n",
        ...
        if (!fCheckOnly) {
          ...
          pcoin->WriteToDisk();
                 ...
                 (txindex.vSpent.size() > n && !txindex.vSpent[n].IsNull())) {
        LogPrintf("FixSpentCoins found spent coin %s OXID %s[%d], %s\n",
                  FormatMoney(pcoin->vout[n].nValue),
                  pcoin->GetHash().ToString(), n,
                  fCheckOnly ? "repair not attempted" : "repairing");
        nMismatchFound++;
  ...
  if (nIndex == -1) {
    ...
    if (nIndex != -1)
      ...
      if (pwallet->vchDefaultKey.IsValid()) {
        LogPrintf("CReserveKey::GetReservedKey(): Warning: Using default key "
                  "instead of a new key, top up your keypool!");
...
void CWallet::GetKeyBirthTimes(std::map<CKeyID, int64_t> &mapKeyBirth) const {
  ...
  CBlockIndex *pindexMax = FindBlockByHeight(std::max(
      0, nBestHeight -
  ...
  for (std::map<uint256, CWalletTx>::const_iterator it = mapWallet.begin();
    ...
    const CWalletTx &wtx = (*it).second;
    std::map<uint256, CBlockIndex *>::const_iterator blit =
  ...
  for (std::map<CKeyID, CBlockIndex *>::const_iterator it =
    ...
    mapKeyBirth[it->first] =
        it->second->nTime - 7200; // block times can be 2h off
}
