                           ...
                           list<pair<CTxDestination, int64_t> >& listSent, CAmount& nFee, string& strSentAccount, const isminefilter& filter) const
{
    ...
    BOOST_FOREACH(const CTxOut& txout, vout)
    {
        ...
        // OXIDNOTE: CoinControl possible fix related... with HD wallet we need to report change?
        ...
        BOOST_FOREACH(int64_t v, darkSendDenominations)
        {
            BOOST_FOREACH(const COutput& out, vCoins)
            {
                if(out.tx->vout[out.i].nValue == v                                            //make sure it's the denom we're looking for
                    && nValueRet + out.tx->vout[out.i].nValue < nTargetValue + (0.1*COIN)+100 //round the amount up to .1OXID over
                ){
                    CTxIn vin = CTxIn(out.tx->GetHash(),out.i);
                    int rounds = GetInputDarksendRounds(vin);
                    // make sure it's actually anonymized
            ...
            while (true)
            {
                ...
                if (!SelectCoins(nTotalValue, wtxNew.nTime, setCoins, nValueIn, coinControl, coin_type, useIX))
                {
                    if(coin_type == ALL_COINS) {
                        ...
                        strFailReason = _(" Unable to locate enough Darksend non-denominated funds for this transaction that are not equal 1000 OXID.");
                    } else {
...
bool CWallet::CreateCoinStake(const CKeyStore& keystore, unsigned int nBits, int64_t nSearchInterval, int64_t nFees, CTransaction& txNew, CKey& key)
{
    ...
    if (pindexPrev->nHeight+1 >= SUPERNODES_START_BLOCK) {
        if (tier != 0) {
            ...
            int64_t baseMasternodeReward = GetHeightAdjustedMasternodeReward(pindexPrev->nHeight+1, masternodeRewards[tier]);
            // Add fees to reward
            masternodePayment = baseMasternodeReward * COIN + (int64_t) (nFees * ((double)baseMasternodeReward/(GetHeightAdjustedPOSReward(pindexPrev->nHeight+1)/COIN + baseMasternodeReward)));
        }
        else {
            masternodePayment = 0;
        }

        int64_t posRewardBigint = GetHeightAdjustedPOSReward(pindexPrev->nHeight+1);

        nCredit += posRewardBigint + nFees;
        LogPrintf("POS + nCredit: %i\n", nCredit);

        if (tier != 0) {
            int64_t mnReward = GetHeightAdjustedMasternodeReward(pindexPrev->nHeight+1, masternodeRewards[tier]);
    ...
    if(hasPayment && txNew.vout.size() == 4 && (payeerewardpercent == 0 || payeerewardpercent == 100)) // 2 stake outputs, stake was split, plus a masternode payment, no reward split
    {
        ...
        LogPrintf("Case 1: masternodePayment=%i blockValue=%i txNew.vout[1].nValue=%d, txNew.vout[2].nValue=%d CENT=%i\n", masternodePayment, blockValue, txNew.vout[1].nValue, txNew.vout[2].nValue, CENT);
    }
    else if(hasPayment && txNew.vout.size() == 3 && (payeerewardpercent == 0 || payeerewardpercent == 100)) // only 1 stake output, was not split, plus a masternode payment, no reward split
    {
        ...
        LogPrintf("Case 2: masternodePayment=%i blockValue=%i txNew.vout[1].nValue=%d\n", masternodePayment, blockValue, txNew.vout[1].nValue);
    }
    else if(hasPayment && txNew.vout.size() == 5 && payeerewardpercent > 0 && payeerewardpercent < 100) // 2 stake outputs, stake was split, plus a masternode payment
    {
        ...
        txNew.vout[1].nValue = (blockValue / 2 / CENT) * CENT;
        txNew.vout[2].nValue = blockValue - txNew.vout[1].nValue;
        LogPrintf("Case 3: masternodePayment=%i blockValue=%i txNew.vout[1].nValue=%d, txNew.vout[2].nValue=%d CENT=%i payeerewardpercent=%i\n", masternodePayment, blockValue, txNew.vout[1].nValue, txNew.vout[2].nValue, CENT, payeerewardpercent);
    }
    else if(hasPayment && txNew.vout.size() == 4 && payeerewardpercent > 0 && payeerewardpercent < 100) // only 1 stake output, was not split, plus a masternode payment
    {
        ...
        blockValue -= masternodePayment;
        txNew.vout[1].nValue = blockValue;
        LogPrintf("Case 4: masternodePayment=%i blockValue=%i txNew.vout[1].nValue=%d, payeerewardpercent=%i\n", masternodePayment, blockValue, txNew.vout[1].nValue, payeerewardpercent);
    } else {
...
bool CWallet::SetAddressBookName(const CTxDestination& address, const string& strName)
{
    ...
    if (!fFileBacked)
        return false;
    return CWalletDB(strWalletFile).WriteName(COxidcoinAddress(address).ToString(), strName);
}
...
bool CWallet::DelAddressBookName(const CTxDestination& address)
{
    ...
    return CWalletDB(strWalletFile).EraseName(COxidcoinAddress(address).ToString());
}
