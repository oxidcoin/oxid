...
void OverviewPage::updateDarksendProgress()
{
    ...
    QString strAnonymizeOxidAmount = BitcoinUnits::formatHtmlWithUnit(nDisplayUnit, nAnonymizeOxidAmount * COIN, false, BitcoinUnits::separatorAlways);
    ...
    if(currentBalance == 0)
    {
        ...
        strAnonymizeOxidAmount = strAnonymizeOxidAmount.remove(strAnonymizeOxidAmount.indexOf("."), BitcoinUnits::decimals(nDisplayUnit) + 1);
        strAmountAndRounds = strAnonymizeOxidAmount + " / " + tr("%n Rounds", "", nDarksendRounds);
    ...
    if(nMaxToAnonymize > nAnonymizeOxidAmount*COIN) nMaxToAnonymize = nAnonymizeOxidAmount*COIN;

    if(nMaxToAnonymize == 0) return;

    if(nMaxToAnonymize >= nAnonymizeOxidAmount * COIN) {
        ui->labelAmountRounds->setToolTip(tr("Found enough compatible inputs to anonymize %1")
                                          .arg(strAnonymizeOxidAmount));
        strAnonymizeOxidAmount = strAnonymizeOxidAmount.remove(strAnonymizeOxidAmount.indexOf("."), BitcoinUnits::decimals(nDisplayUnit) + 1);
        strAmountAndRounds = strAnonymizeOxidAmount + " / " + tr("%n Rounds", "", nDarksendRounds);
    } else {
