...
const string strMessageMagic = "Oxid Signed Message:\n";
...
bool IsMNCollateralValid(int64_t value, int nHeight) {
    if (nHeight < SUPERNODES_START_BLOCK) {
...
int64_t GetMNCollateral(int nHeight, int tier) {
    if (nHeight < SUPERNODES_START_BLOCK) {
        ...
        return masternodeCollateral[tier];
    }
}
                        ...
                        bool* pfMissingInputs, bool fRejectInsaneFee, bool ignoreFees)
{
    ...
    // oxid: coinstake is also only valid in a block, not as a loose transaction
    if (tx.IsCoinStake())
...
int64_t GetProofOfWorkReward(int nHeight, int64_t nFees)
{
    ...
    else if (nHeight >= 1001 && nHeight <= 20000) {
        nSubsidy = 1 * COIN;
    }
    else if (nHeight >= 20001 && nHeight <= 50000) {
        nSubsidy = 2 * COIN;
    }
    else if (nHeight >= 50001 && nHeight <= 100000) {
        nSubsidy = 3 * COIN;
    }
	else {
		nSubsidy = 0 * COIN;
...
bool IsPOSRewardValid(int64_t value, int64_t nFees) {
    ...
    if (nHeight < SUPERNODES_START_BLOCK) {
        ...
        BOOST_FOREACH(PAIRTYPE(const int, int)& tier, masternodeRewards)
        {
            if (value == (GetHeightAdjustedMasternodeReward(nHeight, tier.second) * COIN + GetHeightAdjustedPOSReward(nHeight) + nFees)) {
                return true;
            }
        }

        if (value ==  GetHeightAdjustedPOSReward(nHeight) + nFees) {
            return true;
        }
    }
...
int64_t GetProofOfStakeReward(const CBlockIndex* pindexPrev, int64_t nCoinAge, int64_t nFees)
{
    ...
    if (pindexBest->nHeight+1 > 1000 && pindexBest->nHeight+1 < SUPERNODES_START_BLOCK) {
        nSubsidy = STATIC_POS_REWARD; // 4
    }
	return nSubsidy + nFees;
}
...
SUPERNODE: 5,001 and up

SN: 24 => 5,001-10,000
SN: 36 => 10,001-20,000
SN: 48 => 20,001-262,800
**********************************************************/

int64_t GetHeightAdjustedMasternodeReward(int nHeight, int64_t baseMasternodeReward) // 8 or 48
{
    int64_t ret = 0;

    if (nHeight > 1000 && nHeight < SUPERNODES_START_BLOCK) {
        ret = 3;
    }
    else if (nHeight >= SUPERNODES_START_BLOCK && nHeight <= 10000) {
        ret = baseMasternodeReward * 0.5; // 4 | 24
    }
    else if (nHeight >= 10001 && nHeight <= 20000) {
        ret = baseMasternodeReward * 0.75; // 6 | 36
    }
    else if (nHeight >= 20001 && nHeight <= 262800) { // year 1
        ret = baseMasternodeReward; // 8 | 48
    }
    else if (nHeight >= 262801 && nHeight <= 525600) { // year 2
        ret = baseMasternodeReward / 2; // 4 | 24
    }
    else if (nHeight >= 525601 && nHeight <= 788400) { // year 3
        ret = baseMasternodeReward / 4; // 2 | 12
    }
    else if (nHeight >= 788401) { // year 4 and up
        ret = baseMasternodeReward / 8; // 1 | 6
    }

    LogPrintf("MN reward: %i\n", ret);

    return ret;
}

int64_t GetHeightAdjustedPOSReward(int nHeight)
{
    int64_t ret = 0 * COIN;
...
#ifdef ENABLE_WALLET
// oxid: attempt to generate suitable proof-of-stake
